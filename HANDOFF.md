...
**7. הערת בטיחות — Automation Webhooks** ⚠️
`AUTOMATION_WEBHOOKS` ב-`journeyConfig.ts` מתועדים לצורך המיפוי בלבד ואינם חלק מזרימת ה-Preview או קישורים פוני-לקוח. אין להפעיל אותם במהלך Preview / QA / validation / local testing, מכיוון שהם מחוברים לאוטומציות הקיימות. קוד הניתוב חוסם אותם ממסלול הפעולות הפונה ללקוח. כל שימוש עתידי מכוון בהם במסגרת אינטגרציית הפרודקשן הוא באחריות שכבת האינטגרציה של נתנאל, ולא חלק מזרימת ה-UI.
...
**אין להפעיל את כתובות ה-webhook המתועדות ב-`AUTOMATION_WEBHOOKS` במסגרת Preview / QA / validation / local testing, ואין להשתמש בהן כקישורים ללקוח.**
אלו נקודות קצה של האוטומציות הקיימות, ולא customer-facing URLs. קוד הניתוב חוסם אותן ממסלול התצוגה/פעולות של הלקוח. אם יהיה צורך להשתמש בהן בהמשך כחלק מאינטגרציית הפרודקשן, זה צריך להיעשות באופן מכוון ומבוקר על-ידי שכבת האינטגרציה, לא דרך ה-UI או כלי ה-Preview.
